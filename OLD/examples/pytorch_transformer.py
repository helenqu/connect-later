...
train_iter = WikiText2(split='train')
# val_iter = train_iter[0:3000]#WikiText2(split='valid')
test_iter = WikiText2(split='test')

print("what are the items in wikitext2? are they sentences? articles? is there a stop token at the end of each?")
for i, item in enumerate(train_iter):
    ...
    print("####### end of tokenized item #######")
    print(vocab(tokenizer(item)))
    print("####### end of vocab tokenized item #######")

# concatenated into a single flat tensor
train_data = data_process(train_iter)
val_data = train_data[:3000]#data_process(val_iter)
test_data = data_process(test_iter)

print("after tokenizing and flattening, what does the data look like?")
print(train_data[:50])
...
batch_size = 20
eval_batch_size = 10
train_data = batchify(train_data, batch_size)  # shape [seq_len, batch_size], aka seq_len arrays of batch_size elements each
...
def get_batch(source: Tensor, i: int) -> Tuple[Tensor, Tensor]:
    ...
    data = source[i:i+seq_len] # get batches i through i+seq_len
    target = source[i+1:i+1+seq_len].reshape(-1) # get batches i+1 through i+1+seq_len, and flatten
    return data, target
